      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="table-responsive">
            <table id="reviewTable" class="table card-table table-vcenter text-nowrap datatable">
              <thead>
                <tr>
                  ...
                  <th> {{__('admin.reviews.field_name')}}</th>
                  <th>{{ __('admin.reviews.field_job') }}</th>
                  <th> {{__('admin.reviews.status')}}</th>

                  <th>{{ __('admin.reviews.field_comment') }}</th>

                  <th>{{ __('admin.reviews.actions') }}</th>
                </tr>
              </thead>
              <tbody>
                @foreach($rows as $row)
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <div class="form-check form-switch md-3" style="margin:10px">
                      ...
                      <input data-id="{{$row->id}}" data-type='App\Models\Review' class="form-check-input form-control toggole-class" type="checkbox" style="float: right;" role="switch" id="flexSwitchCheckDefault" @if($row->active==1) checked="checked" @endif name="active">
                    </div>
                  </td>
...
<script>
  ...
  new DataTable('#reviewTable', {
    "createdRow": function(row, data, dataIndex) {
      $('#reviewTable').removeClass('dataTable');
    },
    language: {
