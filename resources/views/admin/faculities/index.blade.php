            ...
            <table id="facultyTable" class="table card-table table-vcenter text-nowrap">
              <thead>
                <tr>
                  ...
                  <th> {{__('admin.faculities.name')}}</th>
                  <th> {{__('admin.faculities.min_degree')}}</th>
                  <th>{{ __('admin.faculities.gender') }}</th>

                  <th>{{ __('admin.faculities.status') }}</th>


                  <th>{{ __('admin.faculities.actions') }}</th>
                </tr>
              </thead>
              <tbody>
                @foreach($rows as $row)
                ...
                <tr>
                  ...
                  <td>{{$row->name}}</td>
                  <td>{{$row->min_accept_degree}}</td>
                  <td>
                    @if($row->gender == 'male')
                    {{ __('admin.male_gender') }}

                    @else
                    {{ __('admin.female_gender') }}
                  ...
                  <td>
                    <div class="form-check form-switch md-3" style="margin:10px">
                      ...
                      <input data-id="{{$row->id}}" data-type='App\Models\Faculty' class="form-check-input form-control toggole-class" type="checkbox" style="float: right;" role="switch" id="flexSwitchCheckDefault" @if($row->active==1) checked="checked" @endif name="active">
                    </div>
                  </td>
