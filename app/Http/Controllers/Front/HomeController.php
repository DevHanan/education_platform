...
class HomeController extends Controller
{
    ...
    {
        $malefaculities = Faculty::active()->where('min_accept_degree', '<=', $request->result2)->where('gender', 'male')->where('gender', 'both')->get();
        $femalefaculities = Faculty::active()->where('min_accept_degree', '<=', $request->result2)->where('gender', 'female')->where('gender', 'both')->get();
